  ...
  try {
    switch (toolName) {
      ...
      case 'review_denied_claims': {
        // Shared with the MCP surface — see server/services/denialReviewService.ts
        return JSON.stringify(await reviewDeniedClaims(practiceId));
      }
      ...
      case 'suggest_claim_correction': {
        ...
        try {
          // Shared with the MCP surface — see server/services/denialReviewService.ts
          return JSON.stringify(await suggestClaimCorrection(practiceId, corrClaimId));
        } catch (err: any) {
          return JSON.stringify({ error: err?.message || 'Failed to suggest claim corrections.' });
        }
      }
      ...
      case 'review_underpayments': {
        ...
        // Shared with the MCP surface — see server/services/underpaymentReviewService.ts
        return JSON.stringify(await reviewUnderpayments(practiceId, daysBack));
      }
      ...
      case 'draft_underpayment_dispute': {
        ...
        try {
          // Shared with the MCP surface — see server/services/underpaymentReviewService.ts
          return JSON.stringify(await draftUnderpaymentDispute(practiceId, disputeClaimId));
        } catch (err: any) {
...
// (generateDisputeLetterText moved to server/services/underpaymentReviewService.ts
//  so the in-app and MCP surfaces emit identical dispute letters.)
