...
export function startScheduler() {
    ...
    try {
      ...
      for (const practiceId of practiceIds) {
        ...
        const text = [
          ...
          `  Denied: ${report.claimTrends.thisWeek.denied} / ${report.claimTrends.lastWeek.denied}`,
          '',
          'REVENUE',
          `  Collected: $${report.revenueSummary.totalCollected.toFixed(2)}`,
          `  Outstanding: $${report.revenueSummary.totalOutstanding.toFixed(2)}`,
          '',
          `Top Denial Reasons: ${report.topDenialReasons.map(r => `${r.reason} (${r.count})`).join(', ') || 'None'}`,
          '',
          'RECOMMENDATIONS',
          recommendations,
          '',
          `Generated by TherapyBill AI`,
        ].join('\n');
        ...
        const html = `
          <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;color:#1a1a1a;">
            ...
            <h3>Revenue</h3>
            <p>Collected: $${report.revenueSummary.totalCollected.toFixed(2)} |
               Outstanding: $${report.revenueSummary.totalOutstanding.toFixed(2)}</p>
            ${report.topDenialReasons.length > 0 ? `<h3>Top Denial Reasons</h3><p>${report.topDenialReasons.map(r => `${r.reason} (${r.count})`).join(', ')}</p>` : ''}
  ...
  scheduledTasks.set('weeklyInsightsReport', weeklyInsightsTask);
